/**
 * @module
 * collection of enumerations, typically used as identifiers	.
 *
 * @license ArchiMate® is a registered trademark of The Open Group. https://www.opengroup.org/archimate-forum/archimate-overview
 *
 */

export { AspectTypes, type AspectTypesUnion } from './aspect-types-enum.js'
export { ElementType } from './element-type-enum.js'
export { Layers } from './layer-enum.js'
export { ResourceClass } from './resource-class-enum.js'
export { RelationshipType } from './relationship-type-enum.js'
export { RelationshipConnectorType } from './relationship-connector-type-enum.js'
export { PropertyTypes, type PropertyTypesUnion } from './property-types-enum.js'